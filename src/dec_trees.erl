...
%%% Takes decimal keys.
%%%
%%% Do not try to use anything else..
%%%===================================================================
...
%%%% I'm in a rush here. Needless to say, comments like the below
%%%% do not apply to this quickly hacked-together version..
%%%% 
%%%% HB
...
lookup_1(Key, {Key1, Value, Smaller, Bigger}) ->
    case dec:cmp(Key, Key1) of
        -1 -> lookup_1(Key, Smaller);
        0 -> {value, Value};
        1 -> lookup_1(Key, Bigger)
    end;
...
is_defined_1(Key, {Key1, _Value, Smaller, Bigger}) ->
    case dec:cmp(Key, Key1) of
        -1 -> is_defined_1(Key, Smaller);
        0 -> true;
        1 -> is_defined_1(Key, Bigger)
    end;
...
get_1(Key, {Key1, Value, Smaller, Bigger}) ->
    case dec:cmp(Key, Key1) of
        -1 -> get_1(Key, Smaller);
        0 -> Value;
        1 -> get_1(Key, Bigger)
...
update_1(Key, Value, {Key1, V, Smaller, Bigger}) ->
    case dec:cmp(Key, Key1) of
        -1 -> {Key1, V, update_1(Key, Value, Smaller), Bigger};
        0 -> {Key, Value, Smaller, Bigger};
        1 -> {Key1, V, Smaller, update_1(Key, Value, Bigger)}
    end.
...
insert_1(Key, Value, {Key1, V, Smaller, Bigger}, S) ->
    case dec:cmp(Key, Key1) of
        ...
        0 ->
            %% The original gb_trees crashes here. HB
...
delete_1(Key, {Key1, Value, Smaller, Larger}) ->
    case dec:cmp(Key, Key1) of
        ...
        1 ->
            Larger1 = delete_1(Key, Larger),
            {Key1, Value, Smaller, Larger1}
    end.
